...
/*
        ...
        LinkedHashMap<Object, Set<IIFCClass>> resultValues = new LinkedHashMap<>();
        double precision = 1e-6;

        for (Object ele0 : elements0) {
            if (ele0 instanceof IIFCClass classInterface0) {

                final var subList = new HashSet<IIFCClass>();
                ...
                for (Object ele1 : elements1) {
                    if (ele1 instanceof IIFCClass classInterface1) {

                        // System.out.println(classInterface1.getStepLine());
                        final var geomShapesB = getSolidShapes(ifcModel, classInterface1);
...
/*
    private ArrayList<SolidShape> getSolidShapes(IIFCModel ifcModel, IIFCClass ci) {
        final var geomShapes = new ArrayList<SolidShape>();
        ...
        if (geomShapes.isEmpty()) {
            if (ci instanceof IfcObjectDefinition.Ifc4) {
                for (IIFCClass ifcObj : getDecomposedElements((IfcObjectDefinition.Ifc4) ci)) {
