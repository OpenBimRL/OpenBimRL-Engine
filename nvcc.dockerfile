...
# NVIDIA Engine image: CUDA toolkit + host clang OpenMP offload (WIP).
#
# Build-time:  IfcOpenShell + JVM fat jar (no native .so).
# Start-time:  CMD compiles native for OPENBIMRL_CUDA_OFFLOAD_ARCH, bundles into
...
#   OPENBIMRL_CUDA_OFFLOAD_ARCH   CUDA target, e.g. sm_89 (must match your GPU).
#
...
# ---------------------------------------------------------------------------
# Runtime: CUDA + native toolchain; CMD compiles .so then runs JVM jar.
# ---------------------------------------------------------------------------
...
ENV PATH=${JAVA_HOME}/bin:${CUDA_HOME}/bin:${PATH}
...
ENV LD_LIBRARY_PATH=/usr/local/cuda/lib64:/opt/ifcopenshell/lib
...
cmake -G Ninja -S \"${OPENBIMRL_NATIVE_SRC}\" -B \"${OPENBIMRL_NATIVE_BUILD}\" \
    ...
    -DCMAKE_BUILD_TYPE=Release \
    -DOPENBIMRL_ENABLE_CUDA_OFFLOAD=ON \
    -DOPENBIMRL_CUDA_OFFLOAD_ARCH=\"${OPENBIMRL_CUDA_OFFLOAD_ARCH}\" \
